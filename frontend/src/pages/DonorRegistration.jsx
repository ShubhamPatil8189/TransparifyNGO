...
const navItems = [
  ...
  { label: "Compaigns", path: "/all-campaigns" },
...
export default function DonorRegistration() {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      {/* Gradient Header (matches DashboardHeader style) */}
      <header
        style={{
          background:
            "linear-gradient(135deg, hsl(199, 89%, 30%) 0%, hsl(160, 84%, 30%) 100%)",
        }}
        className="text-white border-b border-transparent"
      >
            ...
            {navItems.map((item) => (
              <Link
                ...
                className={`text-sm font-medium transition-colors px-3 py-2 rounded-md ${
                  location.pathname === item.path ? "bg-white/12" : "hover:bg-white/8"
                }`}
              >
                {item.label}
              </Link>
